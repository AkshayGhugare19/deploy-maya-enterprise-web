...
const KidneyMedicinesCard = ({ item, stepperProgressCartData, setStepperProgressCartData }) => {
  ...
  return <div className="">
    <div key={item?._id} className="p-4">
      <div className={`border p-4 rounded-[16px] h-[350px] flex flex-col justify-between bg-[#FFFFFF] hover:shadow-xl group ${!item?.productQuantity ? 'opacity-65' : ''}`} >
        {/* Prescription badge */}
          ...
          item?.productQuantity ? (
            !isLoggedIn ? (<button className="w-full flex items-center justify-center bg-gray-200 hover:bg-gray-300 px-7 py-2 rounded-full group-hover:bg-[#14967F] group-hover:text-[#FFFFFF]" onClick={addToCart}>
            ...
            </button>) : <>
              {
                ...
                ) : (loading ? <SimpleLoader /> : <button className="w-full flex items-center justify-center bg-gray-200 px-7 py-2 rounded-full group-hover:bg-[#14967F] group-hover:text-[#FFFFFF]" onClick={(e) => addToCart(item._id)}>
